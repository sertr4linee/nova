...
export function GridNodePulses({
  ...
  useEffect(() => {
    ...
    const getOrganic3DGridPosition = () => {
      ...
      const gridY = Math.floor(Math.random() * rows) + (Math.random() * 0.6 - 0.3);
    ...
    const initialPulses: Pulse[] = Array.from({ length: pulseCount }).map((_, idx) => {
  ...
  }, [pulseCount, columns, rows, cellWidth, cellHeight, minSize, maxSize, connectionProbability]);
  ...
  }, [pulses, connections, pulseColor, activePulses]);
